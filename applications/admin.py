...
class ApplicationMessageInline(admin.TabularInline):
    ...
    extra = 1
    fields = ['sender_name', 'is_from_staff', 'message', 'created_at']
    readonly_fields = ['created_at']


@admin.register(WebsiteApplication)
class WebsiteApplicationAdmin(admin.ModelAdmin):
    list_display = ['project_title', 'get_full_name', 'website_type', 'budget_range', 'status', 'priority', 'created_at']
    list_filter = ['status', 'priority', 'website_type', 'budget_range', 'created_at']
    search_fields = ['first_name', 'last_name', 'email', 'phone', 'project_title', 'project_description']
    ...
    fieldsets = (
        ...
        ('Project Details', {
            'fields': ('website_type', 'project_title', 'project_description')
        }),
        ('Budget & Timeline', {
            'fields': ('budget_range', 'preferred_timeline', 'quoted_price')
        }),
        ('Additional Services', {
            'fields': ('additional_services',)
        }),
        ('Design & Content', {
            'fields': ('design_preferences', 'has_logo', 'has_content', 'has_domain')
        }),
        ('Features', {
    ...
    actions = ['mark_as_under_review', 'mark_as_quoted', 'mark_as_accepted', 'mark_as_in_progress']
    ...
    mark_as_accepted.short_description = "Mark selected as Quote Accepted"
...
class ApplicationMessageAdmin(admin.ModelAdmin):
    list_display = ['application', 'sender_name', 'is_from_staff', 'is_read', 'created_at']
    list_filter = ['is_from_staff', 'is_read', 'created_at']
